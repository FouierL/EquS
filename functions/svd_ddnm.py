...
def ddnm_EquS_plus_diffusion(x, model, b, eta, A_funcs, y, cls_fn=None, classes=None, config=None):
    with torch.no_grad():
        ...
        # generate time schedule
        seqs = get_schedule(config.diffusion.num_diffusion_timesteps, config.time_travel.T_sampling, skip_type='quad')
        times = get_schedule_jump(config.time_travel.T_sampling, 
                               config.time_travel.travel_length, 
                               config.time_travel.travel_repeat,
                              )
        time_pairs = list(zip(times[:-1], times[1:], seqs[:-1], seqs[1:]))
        Tg = transforms.RandomHorizontalFlip(p=1)
        # reverse diffusion sampling
        for i, j, timei, timej in tqdm(time_pairs):
            i, j = i*skip, j*skip
            if j<0: j=-1 

            if j < i: # normal sampling 
                t = (torch.ones(n) * timei).to(x.device)
                next_t = (torch.ones(n) * timej).to(x.device)
                at = compute_alpha(b, t.long())
                at_next = compute_alpha(b, next_t.long())
                xt = xs[-1].to('cuda')
...
def ddnm_plus_EquS_diffusion(x, model, b, eta, A_funcs, y, sigma_y, cls_fn=None, classes=None, config=None):
    with torch.no_grad():
