...
from functions.svd_ddnm import ddnm_diffusion, ddnm_plus_diffusion, ddnm_EquS_diffusion, ddnm_EquS_plus_diffusion, ddnm_plus_EquS_diffusion
...
class Diffusion(object):
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        idx_so_far = args.subset_start
        EquS = args.EquS
        EquSplus = args.EquSplus
        avg_psnr = 0.0
        avg_ssim = 0.0
        avg_lpips = 0.0
        pbar = tqdm.tqdm(val_loader)
        pbar2 = tqdm.tqdm(val_loader)
        pbar3 = tqdm.tqdm(val_loader)
        idx = 0
        loss_fn = LearnedPerceptualImagePatchSimilarity(net_type='vgg').to(self.device)
        for x_orig, classes in pbar:
            x_orig = x_orig.to(self.device)
            x_orig = data_transform(self.config, x_orig)
            ...
            with torch.no_grad():
                if EquS:
                    x, _ = ddnm_EquS_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config)
                elif EquSplus:
                    x, _ = ddnm_EquS_plus_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config)
                elif sigma_y==0.: # noise-free case, turn to ddnm
                    x, _ = ddnm_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config)
                elif sigma_y>0 and EquS: # noisy case, turn to ddnm+
                    x, _ = ddnm_plus_EquS_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, sigma_y, cls_fn=cls_fn, classes=classes, config=config)
                elif sigma_y>0:
                    x, _ = ddnm_plus_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, sigma_y, cls_fn=cls_fn, classes=classes, config=config)
                else:
                    print('We dont support your choice')
            ...
            for j in range(x[0].size(0)):
                ...
                psnr = 10 * torch.log10(1 / mse)
                avg_psnr += psnr
                ssim_i = ssim(x[0][j].cpu().numpy(), orig.cpu().numpy(),data_range=1.0,channel_axis=0)
                lpips_value = loss_fn(x[0][j].unsqueeze(0).to(self.device), orig.unsqueeze(0).to(self.device)).item()
            ...
            pbar.set_description("PSNR: %.2f" % (avg_psnr / (idx_so_far - idx_init)))
            pbar2.set_description("SSIM: %.4f" % (avg_ssim / (idx_so_far - idx_init)))
            pbar3.set_description("LPIPS: %.4f" % (avg_lpips / (idx_so_far - idx_init)))
        avg_psnr = avg_psnr / (idx_so_far - idx_init)
        avg_lpips = avg_lpips / (idx_so_far - idx_init)
        avg_ssim = avg_ssim / (idx_so_far - idx_init)
        print("Total Average PSNR: %.2f\n" % avg_psnr)
        print("Total Average SSIM: %.4f\n" % avg_ssim)
        print("Total Average LPIPS: %.5f\n" % avg_lpips)
        print("Number of samples: %d\n" % (idx_so_far - idx_init))
        output_file = os.path.join(self.args.image_folder, "results.txt")
        with open(output_file, "w") as f:
            f.write("Total Average PSNR: %.2f\n" % avg_psnr)
            f.write("Total Average SSIM: %.4f\n" % avg_ssim)
            f.write("Total Average LPIPS: %.5f\n" % avg_lpips)
